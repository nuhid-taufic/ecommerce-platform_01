  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans text-[#111111]">
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50 p-4 flex flex-col">
            {!user ? (
              <div className="m-auto text-center">
                ...
                <Link
                  href="/login"
                  className="text-xs font-bold uppercase tracking-widest bg-primary text-white px-6 py-2 rounded-full hover:opacity-90 transition-colors"
                >
                  Log In
                </Link>
              </div>
              ...
              <div className="space-y-4">
                ...
                {activeTicket.messages.map((msg: any, idx: number) => (
                  ...
                  >
                    <div
                      className={`p-3 rounded-2xl text-sm max-w-[85%] ${msg.sender === "customer" ? "bg-primary text-white rounded-tr-sm" : "bg-white border border-gray-200 text-gray-800 rounded-tl-sm shadow-sm"}`}
                    >
                      {msg.message}
                    </div>
                  </div>
              ...
              <div className="m-auto w-full">
                ...
                <input
                  ...
                  className="w-full text-sm border border-gray-300 rounded-xl px-4 py-3 mb-3 focus:outline-none focus:border-primary transition-colors bg-white"
                />
              </div>
            )}
          </div>
            ...
            <div className="p-3 bg-white border-t border-gray-200 shrink-0">
              ...
              >
                <input
                  ...
                  className="w-full text-sm border border-gray-300 rounded-full pl-4 pr-12 py-3 focus:outline-none focus:border-primary bg-gray-50"
                />
                <button
                  type="submit"
                  disabled={loading || !message.trim()}
                  className="absolute right-1 top-1 bottom-1 w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center hover:opacity-90 disabled:opacity-50 transition-colors"
                >
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${isOpen ? "bg-gray-200 text-secondary rotate-90 scale-90" : "bg-primary text-white hover:scale-105"}`}
      >
        {isOpen ? (
          <X className="h-6 w-6" />
        ) : (
